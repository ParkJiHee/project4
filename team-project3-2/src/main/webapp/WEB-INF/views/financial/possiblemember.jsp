    ...
    $(function(){          
    	$('#btn-upload').click(function(e){
    	 ...
    	 $('#checkAll').click(function(event) { 
    		...
    		/* 	 if ($("#memberlist input[type=checkbox]").prop("checked",true);  )
				$('#listdel').on('click', function(event)  {
    			 
    			(confirm("정보를 삭제 하시겠습니까?")){
    	    		$('#memberlist > tbody').find("input[name=checkrow]").each(function(){
 ...
 <%--      전체 월별 리스트 --%>
		...
		<div class="card mb-3">
            <div class="card-body">
           	...
           	검색:&nbsp;&nbsp;<input type="text" width="5px" id="membersearch" placeholder="회원명검색">
					...
					<c:forEach var="member" items="${ members }"> <%-- 컨트롤러에서 저장하기 --%> 
						<tbody style="text-align:center">
							<tr class="memberlist">
								...
								<td></td>
								<c:choose>
								<c:when test="${ not empty visitpurposevo.purpose }">	<%-- visitpuposevo를 서버에서 modelattribute로 넘겨야함 --%>
								<td id="purposelist">
